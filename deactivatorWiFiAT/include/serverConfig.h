 ...
 <fieldset>
    <legend> parametros de pulsos </legend>
      <form action="/get" target="hidden-form"><br>
            - la duracion del pulso (valor actual %pulse_size_ms% mseg) :&emsp;&emsp;<input type="range" name="pulse_size_ms" value="100" step=50 min="50" max="2500" 
      ...
      <form action="/get" target="hidden-form">
            - la duracion de la pause (valor actual %pause_size_ms% mseg) :&emsp;<input type="range" name="pause_size_ms" value="100" step=50 min="50" max="2000" 
            oninput="document.getElementById('fPause').innerHTML = this.value" />
           <label id="fPause"></label> ms  
           &emsp;<input type="submit" value="grabar" onclick="message_popup()">
      </form><br>
      <form action="/get" target="hidden-form">
            - el numero de pulsos a enviar (valor actual %pulse_num%) :&emsp;&emsp;&emsp;<input type="range" name="pulse_num" value="1" step=1 min="1" max="15" 
